"use client"
import { deleteTransaction } from '@/actions/accounts';
...
const TransactionTable = ({ transactions }) => {
  ...
  const [sortConfig, setSortConfigue] = useState({
    field: "date",
    direction: "desc",
  });
  ...
  const handleSelectAll = () => {
    setSelectedIds((prev) => prev.length === filteredAndSortedTransactions.length ? [] : filteredAndSortedTransactions.map(transaction => transaction.id));
  }

  const handleSort = (field) => {
    setSortConfigue(current => ({
      field,
  ...
  const {loading:deleteLoading, fn:deleteFn, data: deleted} = useFetch(deleteTransaction);

  const hanldeDeleteFn = () => {
  ...
  useEffect(()=>{
    if(deleted && !deleteLoading){
      toast.error("Tranactions deleted successfully");
    }
    ...
    <div className='space-y-4'>
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        <div className='relative flex-1'>
          <Search className='absolute left-2 tp-2.5 h-8 w-4 text-muted-foreground' />
        ...
        <div className='flex gap-2'>
          ...
          <Select type={recurringFilter} onValueChange={(value) => setRecurringFilter(value)}>
          ...
          </Select>
          {
            ...
            (<div className='flex items-center gap-2'>
              <Button variant="destructive" size="sm" onClick={hanldeDeleteFn}>
